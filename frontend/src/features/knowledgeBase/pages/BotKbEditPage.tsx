...
const BotKbEditPage: React.FC = () => {
  ...
  const [botTheme, setBotTheme] = useState("light");
    ...
    if (!isNewBot) {
      ...
      getMyBot(botId)
        .then((bot) => {
          ...
          // Load UI customization settings if they exist
          setBotTitle((bot as any).uiSettings?.title || "");
          setBotTheme((bot as any).uiSettings?.theme || "light");
        })
        .finally(() => {
          setIsLoading(false);
        });
