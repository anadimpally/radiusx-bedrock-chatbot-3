...
/**
 * Component for displaying a chat list drawer with conversations and bots.
 *
 * @param {Props} props - The properties for the ChatListDrawer component.
 * @returns {JSX.Element} The rendered ChatListDrawer component.
 */
const ChatListDrawer: React.FC<Props> = (props) => {
  const { t } = useTranslation();
  const { getPageLabel } = usePageLabel();
  const { opened, switchOpen } = useDrawer();
  const { conversations, recentlyUsedUnsterredBots } = props;
