...
def _is_conversation_role(role: str) -> TypeGuard[ConversationRoleType]:
    """
    Check if a role string is a valid Bedrock conversation role.

    Args:
        role: The role string to check

    Returns:
        bool: True if the role is valid for Bedrock conversations
    """
...
def is_nova_model(model: type_model_name) -> bool:
    ...
    Returns:
        bool: True if the model is an Amazon Nova model
    """
...
def _prepare_nova_model_params(
    ...
    """
    Prepare inference configuration for Amazon Nova models.

    Nova models require specific parameter formatting that differs from other models.

    Args:
        model: The Nova model ID
        generation_params: Optional generation parameters

    Returns:
        Tuple: A tuple containing the inference configuration and additional model fields
    """
...
def compose_args_for_converse_api(
    ...
    """
    Compose arguments for AWS Bedrock Converse API.

    This function formats the input for the Bedrock Converse API request,
    handling different model requirements, generation parameters, guardrails,
    and tool configurations.

    Args:
        messages: The conversation messages
        model: The model ID to use (Claude, Mistral, etc.)
        instructions: Optional system instructions/prompts
        generation_params: Optional generation parameters (temperature, etc.)
        guardrail: Optional Bedrock guardrails configuration
        grounding_source: Optional grounding source for guardrails
        tools: Optional tools for agent functionality
        stream: Whether to enable streaming response

    Returns:
        ConverseStreamRequestRequestTypeDef: The formatted request for the Converse API
    """
...
def call_converse_api(
    ...
    """
    Call the AWS Bedrock Converse API.

    This function sends the formatted request to the Bedrock Converse API
    and returns the response.

    Args:
        args: The formatted API request arguments

    Returns:
        ConverseResponseTypeDef: The API response
    """
    client = get_bedrock_runtime_client()
...
def calculate_price(
    ...
    """
    Calculate the price for a Bedrock model invocation.
...
def get_model_id(
    ...
    """
    Get the full Bedrock model ID for a model name.

    This function maps friendly model names to the actual model IDs used by Bedrock.
    It also handles cross-region inference when enabled.

    Args:
        model: The friendly model name
        enable_cross_region: Whether to enable cross-region inference
        bedrock_region: The AWS region for Bedrock

    Returns:
        str: The full Bedrock model ID
