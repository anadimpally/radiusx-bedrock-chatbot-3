...
def _fetch_bot_with_permission_check(user: User, bot_id: str) -> BotModel:
    """
    Fetch a bot with permission checks based on user role.

    This helper function handles different permission scenarios:
    - Admins can access any public/shared bot
    - Regular users can only access their own bots

    Args:
        user: The user requesting access to the bot
        bot_id: The ID of the bot to fetch

    Returns:
        BotModel: The requested bot

    Raises:
        RecordNotFoundError: If the bot doesn't exist or the user doesn't have permission
    """
    if user.is_admin():
...
def create_bot_publication(user: User, bot_id: str, bot_publish_input: BotPublishInput):
    """
    Publish a bot as an API.

    This function:
    - Verifies the bot exists and is owned by the user
    - Checks if the bot is shared (required for publication)
    - Ensures the bot isn't already being published
    - Triggers the CodeBuild project to deploy the API infrastructure
    - Updates the bot record with publication information

    Args:
        user: The user requesting the publication
        bot_id: The ID of the bot to publish
        bot_publish_input: Publication configuration including throttling, quota, and CORS settings

    Raises:
        RecordNotFoundError: If the bot doesn't exist
        ValueError: If the bot isn't shared or is already published
        ResourceConflictError: If a publication is already in progress
    """
...
def fetch_bot_publication(user: User, bot_id: str) -> BotPublishOutput:
    """
    Get information about a published bot API.

    Retrieves the current status of the bot publication, including:
    - Deployment status (CodeBuild and CloudFormation)
    - API configuration (throttling, quota, CORS settings)
    - API endpoint and key information

    Args:
        user: The requesting user
        bot_id: The ID of the published bot

    Returns:
        BotPublishOutput: The publication details

    Raises:
        RecordNotFoundError: If the bot doesn't exist
        ValueError: If the bot is not published
    """
...
def remove_bot_publication(user: User, bot_id: str):
    """
    Remove a published bot API.

    This function:
    - Verifies the bot exists and is published
    - Deletes all associated API keys
    - Removes the CloudFormation stack
    - Updates the bot record to remove publication information

    Args:
        user: The user requesting the removal
        bot_id: The ID of the published bot

    Raises:
        RecordNotFoundError: If the bot doesn't exist
        ValueError: If the bot is not published or the publication isn't complete
    """
    bot = _fetch_bot_with_permission_check(user, bot_id)
...
def fetch_api_key(user: User, bot_id: str, api_key: str) -> ApiKeyOutput:
    """
    Retrieve information about an API key for a published bot.

    This function:
    - Verifies the bot exists and is published
    - Checks if the API key is associated with the bot
    - Retrieves the API key details including its value

    Args:
        user: The requesting user
        bot_id: The ID of the published bot
        api_key: The ID of the API key to fetch

    Returns:
        ApiKeyOutput: The API key details

    Raises:
        RecordNotFoundError: If the bot or API key doesn't exist
        PermissionError: If the API key is not associated with the bot
    """
    bot = _fetch_bot_with_permission_check(user, bot_id)
...
def create_new_api_key(
    ...
    """
    Create a new API key for a published bot.

    This function:
    - Verifies the bot exists and is published
    - Creates a new API key with the specified description
    - Associates the API key with the bot's usage plan

    Args:
        user: The requesting user
        bot_id: The ID of the published bot
        api_key_input: Input containing the API key description

    Returns:
        ApiKeyOutput: The created API key details

    Raises:
        RecordNotFoundError: If the bot doesn't exist
        AssertionError: If the bot's stack creation is not complete
    """
    bot = _fetch_bot_with_permission_check(user, bot_id)
...
def remove_api_key(user: User, bot_id: str, api_key_id: str):
    """
    Delete an API key for a published bot.
