...
This module implements the core conversation functionality, handling message
generation, context retrieval, and conversation management. It orchestrates
the interaction between users and AI models.
"""
...
def prepare_conversation(
    ...
    """
    Prepare a conversation for processing a chat message.

    This function handles:
    - Finding or creating a conversation
    - Setting up the message structure
    - Retrieving associated bot information if needed
    - Managing conversation alias creation for shared bots

    Args:
        user_id: The ID of the user sending the message
        chat_input: The input containing message and conversation details

    Returns:
        tuple: A tuple containing:
            - message_id: The ID of the user message
            - conversation: The conversation model
            - bot: The associated bot model, if any

    Raises:
        ValueError: If there's an issue with the conversation structure
    """
...
def trace_to_root(
    ...
    """
    Trace the conversation history from a leaf node back to the root.

    Builds a sequence of messages by traversing the conversation tree from the
    specified node up to the root, including thinking logs from agent interactions.

    Args:
        node_id: The ID of the node to start tracing from
        message_map: The map of all messages in the conversation

    Returns:
        list[SimpleMessageModel]: The ordered list of messages from root to the specified node
    """
...
def chat(
    ...
    This function handles the complete chat process including:
    - Preparing the conversation context
    - Setting up tools for agent-enabled bots
    - Retrieving knowledge base information for RAG
    - Invoking the LLM with proper context
    - Processing tool usage and results for agent workflows
    - Managing conversation state and storage

    Args:
        user_id: The ID of the user sending the message
        chat_input: The chat input containing message and conversation details
        on_stream: Optional callback for streaming tokens as they're generated
        on_stop: Optional callback for when generation completes
        on_thinking: Optional callback for agent thinking steps
        on_tool_result: Optional callback for processing tool results

    Returns:
        tuple[ConversationModel, MessageModel]: The updated conversation and the generated message
    """
...
def chat_output_from_message(
    ...
    """
    Convert an internal message model to the external API response format.

    This function transforms the internal conversation and message models
    into the format expected by API clients.

    Args:
        conversation: The conversation model containing the message
        message: The message model to convert

    Returns:
        ChatOutput: The formatted API response
    """
    return ChatOutput(
        conversation_id=conversation.id,
...
def propose_conversation_title(
    ...
    This function uses an AI model to analyze the conversation history and
    suggest an appropriate title.

    Args:
        user_id: The ID of the user who owns the conversation
        conversation_id: The ID of the conversation to title
        model: The ID of the model to use for title generation

    Returns:
        str: The proposed title for the conversation

    Raises:
        RecordNotFoundError: If the conversation is not found
    """
...
def fetch_conversation(user_id: str, conversation_id: str) -> Conversation:
    ...
    This function retrieves the complete conversation history, including
    all messages and their relationships.
    ...
    Returns:
        Conversation: The conversation with its complete message history
