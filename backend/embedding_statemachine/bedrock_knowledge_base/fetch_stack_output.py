"""
Fetch stack output for Bedrock Knowledge Base.

This script retrieves outputs from a CloudFormation stack associated with a bot's knowledge base,
including knowledge base IDs, data source IDs, and guardrail information.

Environment Variables:
- BEDROCK_REGION: The AWS region for Bedrock services
"""
...
def handler(event: Dict[str, str], context: Any) -> StackResult:
    """
    Lambda handler to fetch stack outputs.

    This function retrieves the outputs from a CloudFormation stack for a bot's knowledge base.

    Args:
        event: The event data containing the primary and sort keys
        context: The Lambda execution context

    Returns:
        StackResult: The stack outputs including knowledge base and data source IDs
    """
    print(event)
    pk = event["pk"]
    sk = event["sk"]
