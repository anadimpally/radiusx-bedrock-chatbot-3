"""
Update bot sync status in the database.

This script updates the synchronization status of a bot in the database,
based on the results of a knowledge base ingestion job or a failed sync attempt.
...
def update_sync_status(
    ...
    """
    Update the synchronization status of a bot.

    This function updates the sync status, reason, and last execution ID in the database.

    Args:
        user_id: The ID of the user who owns the bot
        bot_id: The ID of the bot to update
        sync_status: The new synchronization status
        sync_status_reason: The reason for the sync status
        last_exec_id: The ID of the last execution
    """
...
def extract_from_cause(cause_str: str) -> tuple:
    """
    Extract primary and sort keys from a cause string.

    Args:
        cause_str: The cause string containing environment variables

    Returns:
        tuple: A tuple containing the primary key, sort key, and build ARN
    """
...
def handler(event, context):
    """
    Lambda handler to update bot sync status.

    This function updates the sync status of a bot based on the event data.

    Args:
        event: The event data containing sync status information
